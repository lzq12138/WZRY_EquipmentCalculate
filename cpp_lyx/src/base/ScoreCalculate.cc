...
ScoreDetail
equipment_score(int price, double attributes_score){
...
double
attribute_score(AttributeValue attr){
    ...
    double percentage_factor = percentage_factor_from_idx(idx);
    double percentage_value = attr.get_percentageValue();
    return base_factor * base_value + percentage_factor * percentage_value;
}

double
attribute_attempt_score(AttributeValue attr, double attempt_base_factor, double attempt_extra_factor){
    std::string name = attr.get_name();
    int idx = name2idx(name);
    double base_value = attr.get_extraValue();      // only extra attribute should be calculate
    double percentage_value = attr.get_percentageValue();
    return attempt_base_factor * base_value + attempt_extra_factor * percentage_value;
}
...
static const double percentage_attribute_factor[N_ATTR] = {HP_FACTOR2, MP_FACTOR2, AD_FACTOR2, AP_FACTOR2, PD_FACTOR2, MD_FACTOR2, MS_FACTOR2, PPen_FACTOR2, MPen_FACTOR2, AS_FACTOR2, CR_FACTOR2, CD_FACTOR2, LS_FACTOR2, SV_FACTOR2, CDR_FACTOR2, Tenacity_FACTOR2, HReg_FACTOR2, MReg_FACTOR2};


double
base_factor_from_idx(int idx){
    return base_attribute_factor[idx];
}

double
percentage_factor_from_idx(int idx){
    return percentage_attribute_factor[idx];
}
